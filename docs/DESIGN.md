...
For log sources, the ProofWatch `instrumentation library` can be used to emit OCSF-compliant LogRecords directly to the collector. ProofWatch validates the raw evidence against a standardized OCSF schema, and 
...
* Full Pipeline: This is the most common use case for all sources. Policy Engine (w/ ProofWatch) -> Beacon Collector -> Compass -> Final Destination
...
**Purpose**: An instrumentation library that standardizes and validates policy decision data at the source. It ensures that all data entering the OpenTelemetry pipeline is OCSF-compliant and well-formed.

**Key Responsibilities**:

* Intercepting policy decision data from the application.

* Validating the data against a standardized OCSF schema.

* Translating the data into a standardized OpenTelemetry LogRecord.

* Emitting the LogRecord directly to the Collector via OTLP.
...
**Key Responsibilities**:
> Note: Cache and async pattern are currently unimplemented
